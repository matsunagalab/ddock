...
> **重要な訂正（2026-08-07、codexレビューを受けた再検証）。上の段落の
> 「39.6 % は上限」「到達可能分の96 %」「スコア関数をどう改良しても上がらない」は
> 誤りである。** §5.14.33 で測った**生の Oracle@50 の high は 44.80 %** であり、
> 精密化前＝格子上のposeだけで 39.6 % を **5.2 pp 超えている**。反例が既に自分の
> データの中にあった。
>
> 原因は定義にある。39.6 % は `export_rotation_ceiling.py` が
> **最近接格子回転** $\arg\min_m d(R_m, R^\star)$ で作った**参照値**であって、
> $\max_m \mathrm{DockQ}$ ではない。DockQ は測地距離の単調関数ではなく界面接触で
> 決まるので、角度がやや遠い別の格子点がより良い DockQ を出しうる。正しくは
>
> $$39.6\,\% \;=\; \text{最近接格子点の参照値} \;\le\; \text{格子上の真の上限},$$
>
> であり、真の上限については **≥ 44.80 %** という実測下界しか分かっていない。
>
> 事実として残るのは「精密化（格子の外へ出る操作）が high Oracle を
> 44.80 → 79.60 % へ動かした」ことだけである。これは離散化が重い制約であることを
> 強く示すが、**原因が格子だけであることを証明してはいない**。
> §5.14.33 の誤差分解で high を「精密化の問題」と分類した結論自体は変わらない
> （Oracle が 44.80 → 79.60 と動いた事実に基づくため）。
> 変わるのは「格子が原因と確定した」という因果の主張の強さである。
>
> この誤りは §5.14.31 執筆時点（2026-08-01）には検出できなかった。
> 反証データ（Oracle@50）が出たのは §5.14.33（2026-08-04）で、
> そのとき両者を突き合わせなかったのが見落としである。
...
PDF (`docs/theory_ja.pdf`) も TeX 環境なしで読めるようコミットしてある
（.tex を直したら再ビルドして一緒にコミットすること）。

##### codex によるレビューと、そこで見つかった誤り（2026-08-07）

`ddock_codex` に「正しさのチェック」を依頼し、指摘をすべて自分でコードと
本報告書に当たって検証した。**重大な誤りが8件あり、すべて文書側の誤りだった。**

| # | 誤り | 検証 | 出所 |
| --- | --- | --- | --- |
| 1 | 相関定理の複素共役を受容体側に付けていた | `search.py` はリガンド側に付ける（`F_W * F_L_iface.conj()`）。$t$の符号が逆になる | codex |
| 2 | 「ペア加法性はFFT全探索の**必要**条件」 | 十分条件でしかない。非ペア加法でもチャネル相関に分解できればFFTに乗る | codex |
| 3 | スコアの IFACE 項を $+\sum e_{ij}T_{ij}$ と書いた | `score.py:491` は $W(e)=-e^\top$。正しくは $-\sum e_{ji}T_{ij}$ | codex |
| 3b | $T_{ij}$ の添字を「$i$=受容体, $j$=リガンド」と書いた | `score.py:645` の `einsum("fiv,jv->fij", L, H)` で $i$=**リガンド**, $j$=**受容体**。逆 | 実行者（codexは未指摘） |
| 3c | $T_{ij}$ を厳密な原子対距離カウントと定義 | 実装は格子経由 $T_{ij}=\sum_v L_i(v)H_j(v)$、$H_j$ は6 Å以内の受容体原子数 | codex |
| 4 | 「到達可能な性能の**上限**を線形計画で計算」 | 個別LPの100 %は複合体ごとに専用の $e_c$ を許した値で、共有表の上限ではない。§5.14.29 に既に書いてある注意を文書が落としていた | codex |
| 5 | 「L-BFGSで**大域最適**が得られる」 | 凸性が保証するのは「局所最小=大域最小」まで。非滑らかな数値解は最適性の証明ではない | codex |
| 6 | 「格子の上限39.6 %、到達可能分の96 %」 | **上限ではない**（上記の訂正を参照）。生Oracle@50 high が 44.80 % で反例 | codex |
| 7 | 結果表で top-5 と top-50 を混在させ、2×2分解を適用 | 74.40/72.40/60.40 は K=5、84.00/82.40/73.60 は K=50。2×2 は K=5 の4条件のみ | codex |
| 8 | 部分空間表で full=42.76 % を capacity_series の系列に混ぜた | capacity_series の $n$=637 では full=46.1 %。42.76 % は別レシピで配備中の表。nested比較として不公平 | codex |

**さらに codex が見落とし、実行者が照合中に見つけた誤り。**
文書に「LP上限の学習曲線 $n$=25,50,100,200,400,637 → 30.9, 32.0, 42.5, 44.5, 47.4, 46.7 %」
という表を載せていたが、**この数列は本報告書のどこにも存在しない**。
30.9 % は MILP解の validation 値（§5.14.32 の表）、46.7 % は共有表LPの値であり、
両者を無関係に並べた**出典のない表**だった。実在する `capacity_series.py` の
学習曲線（sym / full、$n$=50〜637）に差し替えた。

軽微な誤りも修正した: SE(3) を直積と書いた（正しくは半直積 $\mathrm{SO}(3)\ltimes\R^3$）、
prior の数値を $\lVert\Delta e\rVert=6.86$/4.70 とした（正しくは 6.61/4.37、§5.14.30）、
最近接角度 9.5° と 9.7° の混在（別測定であることを注記）、
「high には5°以下が必要」（個々の複合体の必要条件ではなく集合の90 %水準の話）、
LSE 恒等式の $N/P$ が集合と混同する記法。

**追加した内容**（codexの改善提案）: 用語表（PSC / IFACE / holo / 固定プール /
retrieval / end-to-end / seed / AUC / NMS / InfoNCE / softplus / LSE / big-M /
L-BFGS / pp）、negative results 各項の**射程**（seed数・ラウンド数・母数・
end-to-end未測定であること）、未解決問題に2項目追加（holo限定であること、
固定プール診断と end-to-end の対応が未検証であること）、
最終結論を「加法性一般の限界ではなく、この具体的構成の限界」に限定。

11ページ、Overfull hbox 1.37 pt（約0.5 mm、視認不能）が1件残る以外は警告なし。
