...
##### 蓄積式プールでの hard-negative mining は、学習が成功すると自己無効化する

さらに構造的な問題がある。`absorb()` はプールに**蓄積**し、今回 cap は
一度も効かなかった（`n_old_neg_evicted = 0`）。したがって hinge は
**過去の最難関に固定され続ける**。採掘が信号を足せるのは
「これまで見たどの pose よりも難しいもの」を新たに見つけたときだけであり、
それは**学習が抑制しようとしている失敗そのもの**である。
round 0 の学習が成功しているほど起きにくくなる。

**実測の 3/148 はその帰結である。** つまり round-1 の null は
「採掘の実装が悪い」でも「探索に新規性がない」でもなく、
**この設計（蓄積式プール + max 駆動の hinge + 同じ探索空間）が持つ構造**である。

この読みは実務的な含意を持つ。蓄積式をやめてラウンドごとにプールを
入れ替える（あるいは cap を十分小さくして古い最難関を追い出す）設計なら、
hinge は毎回「現在のモデルの最難関」に張り直される。ただしそれは
