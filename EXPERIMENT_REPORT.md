...
**新たに明示すべき交絡: 最適化ステップ数。** `--min-steps` は N に比例させている
（1500 / 3409 / 6818）ので、N=1000 は N=220 の 4.5 倍のステップを踏んでいる。
§5.14.26 で目的関数が配備指標とずれていることが分かった以上、
**「ずれた目的関数をより長く最適化した」効果と「データが多い」効果は分離できていない**。
§5.14.23 が優先度6（不要）とした step-budget 対照は、この理由で再び必要になった。
最小の対照は N=1000 を `--min-steps 1500` で学習することで、学習1時間・評価2時間で済む。
...
増えるのは同じ分布の同じ比率のコピーであり、しかも §5.14.25 で見たとおり
**N=220 の時点で3 seedが1複合体も違わない**。推定はすでに収束しており、
同じ分布を足しても動くものが無い。これが「データ量がレバーでない」ことの直接の測定である。
